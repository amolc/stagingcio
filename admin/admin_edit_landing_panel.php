<?php
 ...
 $id = $_REQUEST['id'];
 $res = mysql_query("select * from landing_panel where landing_panel_id = '$id'")or die (mysql_error());
	...
	<div class="col-md-12">
		...
		<?php
			...
			if($_POST['upload_button'] == "Submit")
			{
				...
				$landing_panel_type = mysql_real_escape_string($_POST['type']);
				$landing_panel_message = mysql_real_escape_string($_POST['message']);
				$today_date = mktime(0,0,0,date("m"),date("d"),date("Y"));
				$current_date = date("m/d/Y", $today_date);
				$sql   = "insert into landing_panel(landing_panel_type,landing_panel_message,landing_panel_date)
							values('".$landing_panel_type."','".$landing_panel_message."','".$current_date."')";
			
					$query = mysql_query($sql);
					$landing_panel_id = mysql_insert_id();
					if($query)
					{
				  ...
				  $errors= array();
					foreach($_FILES['files']['tmp_name'] as $key => $tmp_name ){
						...
						$fb_query="INSERT into landing_carousel (landing_panel_id,landing_carousel_img,landing_carousel_date) VALUES('$landing_panel_id','$file_name','$current_date')";
						$desired_dir="landing_images";
				...
				<form role="form" action="<?php $_SERVER["PHP_SELF"];?>" method="post"  enctype="multipart/form-data" class="form-horizontal form-groups-bordered">
					<div class="form-group">
							...
							<div class="col-sm-5">
								<select  id='page_type' name="type" class="form-control">
								<?php
								if($row['landing_panel_type'] =='CIO')
								{
									// $select ="selected";
									echo'<option value="CIO" selected>CIO</option>';
									echo'	<option value="ICT Vendor">ICT Vendor</option>	';
								}
								else  
								{
									echo'<option value="CIO">CIO</option>';
									echo'	<option value="ICT Vendor" selected>ICT Vendor</option>	';
								}
								
								// echo $row['landing_panel_message'];
					...
					<div class="form-group">
							<label for="field-ta" class="col-sm-3 control-label">Carousel Images</label>
					...
					<div class="form-group">
							...
							<div class="form-group" style="margin-left: 291px; width: 506px;">
								<textarea style="width:423px;height:140px;" class="form-control wysihtml5 overview" name="message" id="tour_overview" required><?php echo $row['landing_panel_message']; ?></textarea>
							</div>
