	...
	<div class="col-md-12">
		...
		<?php
			...
			if($_POST['upload_button'] == "Submit"){
				...
				$header_email = mysql_real_escape_string($_POST['header_email']);
				$header_phone = mysql_real_escape_string($_POST['header_phone']);
				$header_bg_image = $_FILES["file"]["name"];

				$today_date = mktime(0,0,0,date("m"),date("d"),date("Y"));
				$current_date = date("m/d/Y", $today_date);
				$result = mysql_query("SELECT header_id FROM header");
						...
						{
							//echo "exist";
							$sql   = "UPDATE header SET header_email='".$header_email."',header_phone='".$header_phone."',header_bg_image='".$header_bg_image."',header_insert_date='".$current_date."'";
							
						}
						else
						{
							//echo "does not exist";
							$sql   = "insert into header(header_email,header_phone,header_bg_image,header_insert_date) values('".$header_email."','".$header_phone."','".$header_bg_image."','".$current_date."')";
					...
					if($query)
					{
						echo "Header Background Image Change successful";
					}
					else 
					{
						echo "error";
				...
				<form role="form" action="<?php $_SERVER["PHP_SELF"];?>" method="post"  enctype="multipart/form-data" class="form-horizontal form-groups-bordered">
					...
					<div class="form-group">
						<label for="field-1" class="col-sm-3 control-label">Header Email</label>

						<div class="col-sm-5">
							<input type="text" class="form-control" id="field-1" name="header_email"  placeholder="" required>
						</div>
					</div>
					<div class="form-group">
						<label for="field-1" class="col-sm-3 control-label">Header phone</label>

						<div class="col-sm-5">
							<input type="text" class="form-control" id="field-1" name="header_phone"  placeholder="" required>
						</div>
					</div>
					<div class="form-group">
