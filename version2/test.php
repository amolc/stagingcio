                            ...
                            <div id="carousel">
							   <?php 
								...
								while ($video_res = mysql_fetch_array($video_query))
								{
									...
									$sql_tmp2 = mysql_query("SELECT * FROM videos_type WHERE video_id = '".$video_res['video_id']."'");
									$row_tmp2 =  mysql_fetch_array($sql_tmp2);
																
									?>
								
											<div>
											
											<?php if ($row_tmp2['types'] == "video"){ ?>
												<video style="cursor: pointer;" id="example_video_1" class="video-js vjs-default-skin"
												controls preload="none" width="737" height="415"
												poster="admin/upload/Intro_frame_01.jpg"
												> 
													<?php 
													while( $row_tmp =  mysql_fetch_array($sql_tmp)):
													if(!empty( $row_tmp['path'])){
													$video = $row_tmp['path'];
										...
										{
											if(!empty($row_tmp2['path'])){
										echo '<a href="enter.php"><img style="height: 415px;width: 737px;" src="admin/'.$row_tmp2['path'].'" alt="" /></a>';
										}
